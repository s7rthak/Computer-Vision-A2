...
def jacobian_affine(x_shape,y_shape):
...
def affine_tracker(img_template, img_search, initial_p, coordinate, dimension, max_iter=500):
    
    warp_mat_1 = np.array([[1+ initial_p[0],initial_p[2],initial_p[4]],[initial_p[1],1+initial_p[3],initial_p[5]]] )
    warp_mat_1 = warp_mat_1.astype(np.float32)
    store_img = cv2.warpAffine(img_template,warp_mat_1,(img_template.shape[1],img_template.shape[0]),flags=cv2.INTER_CUBIC)
    template = crop(store_img,coordinate,dimension)
    ...
    while(iterations<max_iter):
        ...
        jacob = jacobian_affine(cols,rows)
...
def jacobian_projection(parameters,x_shape,y_shape):
    x = np.array(range(x_shape))
    y = np.array(range(y_shape))
    x,y = np.meshgrid(x, y)
    ones = np.ones((y_shape, x_shape))
...
def projective_tracker(img_template, img_search, initial_p, coordinate, dimension, max_iter=500):
    
    warp_mat_1 = np.array([[1+ initial_p[0], initial_p[3], initial_p[6]],[initial_p[1],1+initial_p[4], initial_p[7]],[initial_p[2], initial_p[5], 1+ initial_p[8]]])
    warp_mat_1 = warp_mat_1.astype(np.float32)
    store_img = cv2.warpPerspective(img_template,warp_mat_1,(img_template.shape[1],img_template.shape[0]),flags=cv2.INTER_CUBIC)
    template = crop(store_img,coordinate,dimension)
    rows, cols = template.shape
    ...
    while(iterations<max_iter):
        ...
        #Applying Lukas Kanade Formula
        jacob = jacobian_projection(parameters,cols,rows)
        steepest_descent = np.matmul(gradient_fin,jacob)
        steepest_descent_T = np.rollaxis(steepest_descent,3,2)
        hessian = np.matmul(steepest_descent_T,steepest_descent).sum((0,1))
        hess_inv = np.linalg.pinv(hessian)
...
for i in range(2,351):
    ...
    #warp_mat = np.array([[1+ parameters[0], parameters[3], parameters[6]],[parameters[1],1+parameters[4], parameters[7]],[parameters[2], parameters[5], 1+ parameters[8]]])
    #warp_mat = warp_mat.astype(np.float32)
