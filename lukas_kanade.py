...
def crop(image,coordinate,dimension):
    return image[(int)(coordinate[0]):(int)(coordinate[0]+dimension[0]),int(coordinate[1]):int(coordinate[1]+dimension[1])]
...
for i in range(2,585):
    ...
    parameters = translation_tracker(store_img_1,store_img_2,initial_parameters,initial_rect,dimensions,learning_rate=1)
    sum1 += store1
